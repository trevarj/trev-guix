(define-module (trev-guix services flatpak)
  #:export (home-flatpak-configuration home-flatpak-configuration?
...
(define-record-type* <home-flatpak-configuration> home-flatpak-configuration
...
(define (home-flatpak-script _)
  (list (local-file (string-append (getenv "HOME")
  ...
  (let* ((remotes (home-flatpak-configuration-remotes config))
    ...
    #~(begin
        ...
        (define (warn action target)
          (format (current-error-port) "warning: flatpak ~a failed for ~a\n"
                  action target))
        ...
        #$@(map (match-lambda
                  ((name . location) #~(add-remote #$name
                                                   #$location))) remotes)
        #$@(map (lambda (application)
                  #~(install-application #$application)) applications))))
